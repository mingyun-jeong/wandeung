...
| Podfile 최소 iOS 버전 | **미설정 (주석 처리)** |
| Google Maps iOS API | **미활성화** |
| FFmpeg 폰트 경로 iOS 분기 | **미설정** |
| R2 시크릿 키 보안 | **위험 (클라이언트 노출)** |
| App Store Connect 등록 | **미등록** |
...
- [ ] Supabase Dashboard에서 Apple OAuth 설정

### Phase 2: 코드 수정

- [ ] `Podfile` — `platform :ios, '14.0'` 설정
- [ ] `Info.plist` — URL Scheme 추가 (reversed client ID)
- [ ] `.env` — `GOOGLE_IOS_CLIENT_ID` 추가
- [ ] `auth_provider.dart` — iOS용 `clientId` 분기
- [ ] `main.dart` — FFmpeg 폰트 경로 iOS 분기
- [ ] Apple 로그인 구현 (sign_in_with_apple 패키지)

### Phase 3: 보안 (프로덕션 배포 전)

- [ ] R2 시크릿 키를 서버 사이드(Edge Function)로 이관
- [ ] 클라이언트 `.env`에서 R2 시크릿 제거
